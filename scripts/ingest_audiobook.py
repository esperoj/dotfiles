...
def ingest_audiobook(identifier):
    ...
    metadata["include_reg"] = "^.*\\.m4a$"
    audiobooks = get_database("primary").get_table("audiobooks")
    record = audiobooks.create(metadata)
    if record:
